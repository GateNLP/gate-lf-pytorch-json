...
class SeqTaggerLSTMSimple(CustomModule):
    ...
    def get_init_weights(self, batchsize):
        ...
        h0.zero_()
        c0.zero_()
        # or we set to small uniformly distributed random numbers
        #h0.copy_(torch.rand(self.layer_lstm.num_layers, batchsize, self.layer_lstm.hidden_size)*0.01)
        #c0.copy_(torch.rand(self.layer_lstm.num_layers, batchsize, self.layer_lstm.hidden_size)*0.01)
        return h0, c0
